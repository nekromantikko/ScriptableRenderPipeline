...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    public class SharedRTManager
    {
        ...
        public void InitSharedBuffers(GBufferManager gbufferManager, RenderPipelineSettings settings, RenderPipelineResources resources)
        {
            ...
            m_CameraDepthStencilBuffer = RTHandles.Alloc(Vector2.one, depthBufferBits: DepthBits.Depth32, colorFormat: RenderTextureFormat.Depth, dimension: TextureDimension.Tex2DArray, name: "CameraDepthStencil");
            ...
            m_CameraDepthBufferMipChain = RTHandles.Alloc(ComputeDepthBufferMipChainSize, colorFormat: RenderTextureFormat.RFloat, dimension: TextureDimension.Tex2DArray, sRGB: false, enableRandomWrite: true, name: "CameraDepthBufferMipChain");

            // Technically we won't need this buffer in some cases, but nothing that we can determine at init time.
            m_CameraStencilBufferCopy = RTHandles.Alloc(Vector2.one, depthBufferBits: DepthBits.None, colorFormat: RenderTextureFormat.R8, dimension: TextureDimension.Tex2DArray, sRGB: false, enableRandomWrite: true, name: "CameraStencilCopy"); // DXGI_FORMAT_R8_UINT is not supported by Unity

            if (m_VelocitySupport)
            {
                m_VelocityRT = RTHandles.Alloc(Vector2.one, colorFormat: Builtin.GetVelocityBufferFormat(), dimension: TextureDimension.Tex2DArray, sRGB: Builtin.GetVelocityBufferSRGBFlag(), name: "Velocity");
                if (m_MSAASupported)
                {
                    m_VelocityMSAART = RTHandles.Alloc(Vector2.one, colorFormat: Builtin.GetVelocityBufferFormat(), dimension: TextureDimension.Tex2DArray, sRGB: Builtin.GetVelocityBufferSRGBFlag(), enableMSAA: true, bindTextureMS: true, name: "VelocityMSAA");
                }
            }
            ...
            if (m_MSAASupported)
            {
                ...
                m_CameraDepthStencilMSAABuffer = RTHandles.Alloc(Vector2.one, depthBufferBits: DepthBits.Depth24, colorFormat: RenderTextureFormat.Depth, dimension: TextureDimension.Tex2DArray, bindTextureMS: true, enableMSAA: true, name: "CameraDepthStencilMSAA");
                m_CameraDepthValuesBuffer = RTHandles.Alloc(Vector2.one, colorFormat: RenderTextureFormat.ARGBFloat, dimension: TextureDimension.Tex2DArray, sRGB: false, name: "DepthValuesBuffer");
                m_DepthAsColorMSAART = RTHandles.Alloc(Vector2.one, colorFormat: RenderTextureFormat.RFloat, dimension: TextureDimension.Tex2DArray, sRGB: false, bindTextureMS: true, enableMSAA: true, name: "DepthAsColorMSAA");
                ...
                m_NormalMSAART = RTHandles.Alloc(Vector2.one, colorFormat: RenderTextureFormat.ARGB32, dimension: TextureDimension.Tex2DArray, sRGB: false, enableMSAA: true, bindTextureMS: true, name: "NormalBufferMSAA");
            ...
            if (!m_ReuseGBufferMemory)
            {
                ...
                m_NormalRT = RTHandles.Alloc(Vector2.one, colorFormat: RenderTextureFormat.ARGB32, dimension: TextureDimension.Tex2DArray, sRGB: false, enableRandomWrite: true, name: "NormalBuffer");
            }
            else
            {
