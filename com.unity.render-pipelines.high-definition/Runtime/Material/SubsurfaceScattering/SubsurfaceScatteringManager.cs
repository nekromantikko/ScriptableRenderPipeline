    ...
    public class SubsurfaceScatteringManager
    {
        ...
        public void InitSSSBuffers(GBufferManager gbufferManager, RenderPipelineSettings settings)
        {
            ...
            if (settings.supportedLitShaderMode == RenderPipelineSettings.SupportedLitShaderMode.ForwardOnly) //forward only
            {
                ...
                m_ColorMRTs[0] = RTHandles.Alloc(Vector2.one, colorFormat: RenderTextureFormat.ARGB32, dimension: TextureDimension.Tex2DArray, sRGB: true, name: "SSSBuffer");
            ...
            if (m_MSAASupport)
            {
                 m_ColorMSAAMRTs[0] = RTHandles.Alloc(Vector2.one, colorFormat: RenderTextureFormat.ARGB32, dimension: TextureDimension.Tex2DArray, enableMSAA: true, bindTextureMS: true, sRGB: true, name: "SSSBufferMSAA");
            }
            ...
            if (NeedTemporarySubsurfaceBuffer() || settings.supportMSAA)
            {
                ...
                m_CameraFilteringBuffer = RTHandles.Alloc(Vector2.one, colorFormat: RenderTextureFormat.RGB111110Float, dimension: TextureDimension.Tex2DArray, sRGB: false, enableRandomWrite: true, name: "SSSCameraFiltering"); // Enable UAV
            }

            // We use 8x8 tiles in order to match the native GCN HTile as closely as possible.
            m_HTile = RTHandles.Alloc(size => new Vector2Int((size.x + 7) / 8, (size.y + 7) / 8), dimension: TextureDimension.Tex2DArray, colorFormat: RenderTextureFormat.R8, sRGB: false, enableRandomWrite: true, name: "SSSHtile"); // Enable UAV
        }
