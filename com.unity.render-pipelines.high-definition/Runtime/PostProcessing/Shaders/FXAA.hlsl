...
float3 Load(TEXTURE2D(_InputTexture), int2 icoords, int idx, int idy)
{
...
{
    {
        ...
        // Blur
        float3 rgb03 = Fetch(_InputTexture, _InputTextureSampler, positionNDC, dir * (0.0 / 3.0 - 0.5));
        float3 rgb13 = Fetch(_InputTexture, _InputTextureSampler, positionNDC, dir * (1.0 / 3.0 - 0.5));
        float3 rgb23 = Fetch(_InputTexture, _InputTextureSampler, positionNDC, dir * (2.0 / 3.0 - 0.5));
        float3 rgb33 = Fetch(_InputTexture, _InputTextureSampler, positionNDC, dir * (3.0 / 3.0 - 0.5));
